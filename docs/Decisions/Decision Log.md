...
| [[ADR-0033 - Volunteer Crawling, Verified Before It Is Believed]] | accepted 2026-08-29 | Volunteers may crawl for the index; a node supplies evidence (text, hashes, timings) and never conclusions, hosts are leased exclusively so politeness stays global, everything lands in quarantine, and verification is sampled by a standing that falls fast and rises slowly |
| [[ADR-0034 - Volunteer GPUs Do Batch Work, Never a Reader's Query]] | accepted 2026-08-29, narrows 0029 for volunteers | Community GPUs run batch jobs over already-crawled content (embeddings, OCR, descriptions, transcripts, cached summaries); no live search path, no query text and no identity ever reaches a volunteer machine; models pinned by digest, results verified by canaries and duplicates |
| [[ADR-0035 - Sharding in Our Own Router]] | accepted 2026-08-29 | Meilisearch's own sharding is Enterprise-only, so the API shards: rendezvous hashing on the document id, fan-out and merge before the existing re-rank, writes routed by id, a dead shard degrades rather than fails, each shard keeping the PROB-004 memory rule |
