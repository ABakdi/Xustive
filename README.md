...
| **Federation** | Optional query-time enrichment from a self-hosted SearXNG through a single gateway — the one allow-listed egress, off by a runtime switch. Since M13 every federated hit is *distilled*: indexed at once, crawled first, and endorsed on its document so it leads the page next time — a page we crawled ourselves that the web also returns is endorsed the same way (ADR-0031). | [Federation Gateway](docs/Components/Federation%20Gateway.md) · [ADR-0017 - Query-Time Federation with External Metasearch](docs/Decisions/ADR-0017%20-%20Query-Time%20Federation%20with%20External%20Metasearch.md) |
